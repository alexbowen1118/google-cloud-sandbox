...
for($j=0;$j<count($fieldName);$j++)
	...
	//	if($pos===false || $zz==1){$z="";}else{$z="DISABLED";}// disable Inventory after week 1
		//$val=number_format($val);
		echo "<td align='right'><input type='text' name='$name' value='$val' size='3'></td>";
		}// end $i
	...
	if($fieldName[$j]=="lake_level"&&@$varLake>0)
		{$val=($val/$varLake);}
	if($name=="comm_hi_temp_total"&&$val>0)
		{
		$val=round(($val/$varTempHi),1);
		$avg_hi=$val;
		}
	if($key=="comm_low_temp"&&$val>0)
		{
		$val=round(($val/$varTempLo),1);
		$avg_lo=$val;
		}
	if($key=="river_level_high"&&$val>0)
		{$val=round(($val/$varRiverHi),1);}
	if($key=="river_level_low"&&$val>0)
		{$val=round(($val/$varRiverLo),1);}

	if(in_array($key,$dec1array))
		{
		if($key=="vol_hours"||$key=="csw_hours")
			{$val_format=number_format($val,1);}
			else
			{$val_format=number_format($val,2);}
		}
		else
		{$val_format=number_format($val);}
	echo "<td><input type='text' name='$name' value='$val' size='7' DISABLED></td>";
	echo "</tr>";
	}// end $j
