...
//	echo "<pre>"; print_r($_POST); echo "</pre>";  //exit;
	...
	$query = "INSERT INTO mfm set $query";
//	echo "$query";exit;
	$result = mysqli_query($connection,$query) or die ("Couldn't execute query Update. $query");
...
//	echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
	...
	$query = "UPDATE mfm set $query where id='$_POST[id]'";
//	echo "$query";exit;
	$result = mysqli_query($connection,$query) or die ("Couldn't execute query Update. $query");
	header("Location:edit_motor_fleet.php?id=$id&m=1");
	exit;
	}
...
if(@$submit=="Remove")
	{
		$table=$_POST['table'];
	$query = "DELETE FROM mfm where id='$_POST[id]'";
	//echo "$query";exit;
	$result = mysqli_query($connection,$query) or die ("Couldn't execute query Update. $query");
...
$sql = "SHOW COLUMNS FROM mfm";//echo "$sql";
...
// ************ toggle district - region ************
$skip=array("id","region");
...
echo "<form name='frmEdit' action=\"edit_motor_fleet.php\" method=\"post\">";
echo "<tr><td align='center' colspan='2'>Motor Fleet Vehicle</td></tr></table>
</div>";
...
$sql= "SELECT * from mfm where id='$id'";
...
$read_only=array("id","active","district","region","park_or_section","center","license_plate","year_make_model","class","vehicle","assign_type","rate_per_mile");
	...
	foreach($row as $k=>$v){
		...
		if($k=="district" AND $level>3)
			...
			if(array_key_exists($row['park_or_section'], $district))
				{
				$v=$district[$row['park_or_section']];
				}
			$input="<select name='$k'>";
			foreach($menu_array as $k1=>$v1)
				{
...
array_unshift($menu_array,"ADMN","DEDE","FAMA","OPAD","RALE","REMA","TRED","TRWD","TRND","EADI","NODI","SODI","WEDI","CORE","PIRE","MORE","INED","STPA","NARA","GRSS","LAND");
			$input="<select name='$k'>";
			foreach($menu_array as $k1=>$v1)
				{
				if($v1==$v){$s="selected";}else{$s="";}
...
<input type='hidden' name='table' value='motor_fleet'>
<input type='hidden' name='id' value='$id'>
<input type='submit' name='update' value='$update_value'>
</td></tr>";
$sub="$row[park_or_section] Motor Fleet vehicle with plate $row[license_plate]";
