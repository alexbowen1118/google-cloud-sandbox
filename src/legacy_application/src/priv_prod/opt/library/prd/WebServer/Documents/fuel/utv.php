...
if(@$add=="Add")
	...
	//echo "<pre>"; print_r($_POST); echo "</pre>";  //exit;
		...
		foreach($_POST as $k=>$v){
			...
			$query=rtrim($query,",");
	$query = "INSERT INTO utv set $query"; //echo "$query";exit;
	...
	$utv_id="utv".$vi;
	$query = "UPDATE utv set utv_id='$utv_id' where id='$id'";
	//echo "$query";exit;
	$result = mysqli_query($connection, $query) or die ("Couldn't execute query Update. $query");
	
	header("Location: menu.php?form_type=utv");
	exit;
	}
...
echo "<tr><td align='center' colspan='2'>utv SPECIFICATIONS
<a onclick=\"toggleDisplay('show_form');\" href=\"javascript:void('')\"><br />Add a UTV</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick=\"toggleDisplay('search_form');\" href=\"javascript:void('')\">Search</a></td>";
...
if($parkList[0]!="")
	...
	foreach($parkList as $k=>$v){
		$con1="menu.php?form_type=utv&park_code=$v";
...
// Input Form
echo "<div align='center' id=\"show_form\" style=\"display: none\"><table border='1' cellpadding='5'><tr><form name='frmTest' action=\"utv.php\" method=\"post\" onsubmit=\"return radio_button_checker()\"><td align='center' colspan='2'>$park_code</td></tr>";
...
// Search Form
echo "<div align='center' id=\"search_form\" style=\"display: none\"><table border='1' cellpadding='5'><tr><form name='frmSearch' action=\"menu.php?form_type=utv\" method=\"post\"><td align='center' colspan='2'>$park_code</td></tr>";
...
if(empty($park_code) AND $_SERVER['QUERY_STRING']=="form_type=utv")
	{
//	EXIT;
	}
...
if(isset($ARRAY))
	...
	$skip1=array("id","comment");
		...
		if($level>3)
			{
			$export_all="<a href=utv_excel.php?search=Find&rep=x>All Parks</a>";
			}
			else
			{$export_all="";}
		
		echo "<div align='center'><table border='1' cellpadding='5'>";
			echo "<tr><th colspan='8'>utv - $num utvs</td><th colspan='4'>Excel export <a href=utv_excel.php?center_code=$center_code&search=Find&rep=x>Park</a> $export_all</th></tr>";
		...
		switch($k)
			{
			case 'center_code';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=cc$passWhere'>center code</a>";
				break;
			case 'park_id';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=p$passWhere'>License Plate</a>";
				break;
			case 'mileage';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=mi$passWhere'>$k</a>";
				break;
			case 'make';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=m$passWhere'>$k</a>";
				break;
			case 'model_year';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=my$passWhere'>$k</a>";
				break;
			case 'duty';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=d$passWhere'>$k</a>";
				break;
			case 'trans';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=t$passWhere'>$k</a>";
				break;
			case 'drive';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=dr$passWhere'>$k</a>";
				break;
			case 'fuel';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=f$passWhere'>$k</a>";
				break;
			case 'emergency';
				$k="<a href='menu.php?search=Find&form_type=utv&sort=e$passWhere'>$k</a>";
				break;
	 //   	default;
	//        	$k="<a href='menu.php?form_type=utv&s=e'>$k</a>";
	 //   		break;
			}
