...
if(@$add=="Add")
	...
	//echo "<pre>"; print_r($_POST); echo "</pre>";  //exit;
		...
		foreach($_POST as $k=>$v){
			...
			$query=rtrim($query,",");
	$query = "INSERT INTO atv set $query"; //echo "$query";exit;
	...
	$atv_id="ATV".$vi;
	$query = "UPDATE atv set atv_id='$atv_id' where id='$id'";
	//echo "$query";exit;
	$result = mysqli_query($connection, $query) or die ("Couldn't execute query Update. $query");
	
	header("Location: menu.php?form_type=atv");
	exit;
	}
...
echo "<tr><td align='center' colspan='2'>ATV SPECIFICATIONS
<a onclick=\"toggleDisplay('show_form');\" href=\"javascript:void('')\"><br />Add a atv</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick=\"toggleDisplay('search_form');\" href=\"javascript:void('')\">Search</a></td>";
...
// For Level 1 with privileges
	if(@$parkList[0]!=""){
		...
		foreach($parkList as $k=>$v){
			$con1="menu.php?form_type=atv&park_code=$v";
...
echo "<div align='center' id=\"show_form\" style=\"display: none\"><table border='1' cellpadding='5'><tr><form name='frmTest' action=\"atv.php\" method=\"post\" onsubmit=\"return radio_button_checker()\"><td align='center' colspan='2'>$park_code</td></tr>";
...
// Search Form
echo "<div align='center' id=\"search_form\" style=\"display: none\"><table border='1' cellpadding='5'><tr><form name='frmSearch' action=\"menu.php?form_type=atv\" method=\"post\"><td align='center' colspan='2'>$park_code</td></tr>";
...
if(!$_POST AND $_SERVER['QUERY_STRING']=="form_type=atv"){EXIT;}
...
if(isset($ARRAY))
	...
	if($level>3){$export_all="<a href=atv_excel.php?search=Find&rep=x>All Parks</a>";}
	
	if(!isset($center_code)){$center_code="";}
	echo "<div align='center'><table border='1' cellpadding='5'>";
		echo "<tr><th colspan='7'>ATV - $num atvs</td><th colspan='4'>Excel export <a href=atv_excel.php?center_code=$center_code&search=Find&rep=x>Park</a> $export_all</th></tr>";
	...
	switch($k)
		{
		case 'center_code';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=cc$passWhere'>center code</a>";
			break;
		case 'park_id';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=p$passWhere'>License Plate</a>";
			break;
		case 'mileage';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=mi$passWhere'>$k</a>";
			break;
		case 'make';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=m$passWhere'>$k</a>";
			break;
		case 'model_year';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=my$passWhere'>$k</a>";
			break;
		case 'duty';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=d$passWhere'>$k</a>";
			break;
		case 'trans';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=t$passWhere'>$k</a>";
			break;
		case 'drive';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=dr$passWhere'>$k</a>";
			break;
		case 'fuel';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=f$passWhere'>$k</a>";
			break;
		case 'emergency';
			$k="<a href='menu.php?search=Find&form_type=atv&sort=e$passWhere'>$k</a>";
			break;
	//   	default;
	//        	$k="<a href='menu.php?form_type=atv&s=e'>$k</a>";
	//   		break;
		}
