...
foreach($list_of_parks_array as $k=>$v)
	{
	if(!in_array($v,$check_park_array))
		{
		$missing[]=$v;
		}
	}
...
// 	echo "<tr><th>NC STATE<BR>PARK</th><th>$mF<br>$y</th><th>TOTAL YTD<BR>$mF $y</th><th>$mF<br>$ly</th><th>TOTAL YTD<BR>$mF $ly</th><th>% CHANGE<br>($y/$ly)<br>$mon &nbsp; YTD</th></tr>";
	echo "<tr><th>NC STATE<BR>PARK</th><th>$mF<br>$y</th><th>$mF<br>$ly</th><th>% CHANGE<br>($y/$ly)<br>$mon</th><th>Justification</th></tr>";
		}
...
// "OCMO","MOJE",
$skipParks=array("BAIS","BATR","BECR","BEPA","BULA","DERI","LOHA","LEIS","MIMI","PIBO","BUMO","RUHI","SARU","SCRI","SUMO","THRO","WOED","YEMO");
...
if(array_key_exists($parkNameYear[$i],$park_comments))
	...
	$var_comments="";
		...
		$csv_array[]=array("$parkLink", "$monthPark", "$monthPreYearPark", "$perCentChangeM%", "$var_comments");
...
// 			<td align='right'>$yearParkPreY</td>
		...
		if(empty($xls))
			...
			<td align='center'>$var_comments</td>
			</tr>";
			}
		@$j++; if($j==2){$j=0;}
		}
...
// 	echo "test<pre>"; print_r($csv_array); echo "</pre>"; // exit;
	...
	$yearPrevTotAll=number_format($yearPrevTotAll,0);
		...
		$csv_array[]=array("", "$parkMonthTot", "$yearPrevTot", "$perCentChangeMtot% ");
	if(empty($xls))
			{
	...
	$header_array[]=array("NC STATE PARK", " $mF $y",  " $mF $ly",  "% CHANGE ($y/$ly)", "Justification");
