...
$database="fuel";
$admin_position_number=$_SESSION[$database]['beacon_num'];
...
// 	echo "<pre>"; print_r($_POST); echo "</pre>";  exit;
		...
		foreach($_POST as $k=>$v){
			...
			$query=rtrim($query,",");
	$query = "INSERT INTO vehicle set $query"; 
//	echo "$query";exit;
	$result = mysqli_query($connection,$query) or die ("Couldn't execute query $query. Error: ". mysqli_error($connection));
	...
	//echo "$query";exit;
	$result = mysqli_query($connection,$query) or die ("Couldn't execute query Update.");
...
// For Level 1 with privileges
	if($parkList[0]!="")
		{
			if(!empty($park_code) AND in_array($park_code,$parkList))
				{
...
// Search Form
echo "<div align='center' id=\"search_form\" style=\"display: none\">
<form name='frmSearch' action=\"menu.php?form_type=inventory\" method=\"post\">
...
echo "<tr>";
// no longer used SURP is now included in the Current Park drop-down
// echo "<td align='center' bgcolor='aliceblue'>
// Check to ONLY view surplused vehicles. <input type='checkbox' name='surplus' value='x'></td>";
echo "<td align='center' bgcolor='green' colspan='2' align='center'><input type='submit' name='search' value='Find'>
</td></tr>";
echo "</table></form></div>";
...
// 		@$where.=" and region='$region'";	
		...
		{
// 		$region=$_SESSION['fuel']['select'];	
		$district=$_SESSION['fuel']['select'];		
		@$where.=" and center_code='$center_code'";	
		}
...
// if(!empty($region) AND empty($sort)){$orderBy="order by center_code";}
...
//  $sql= "SELECT t4.region as region, $t1_fields, t3.Lname
// from vehicle as t1
...
// LEFT JOIN dpr_system.parkcode_names_region as t4 on t4.park_code=t1.center_code
//  where $where0
//  $where
// $where_dist
//  $orderBy
...
LEFT JOIN dpr_system.parkcode_names_district as t4 on t4.park_code=t1.center_code
 ...
 if($level>4)
 	{
//  	echo "$sql<pre>"; print_r($_REQUEST); echo "</pre>";
 	}
...
if(@$ARRAY)
	...
	if($level>3)
		{
		$export_all="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=inventory_csv.php?search=Find&rep=x>All Parks</a>";
		}
		else
		{$export_all="";}
		...
		echo "<div align='center'><table border='1' cellpadding='5'>";
			echo "<tr><th colspan='10'>On-Road Inventory - $num vehicles</th><th colspan='4'>Excel export <a href=inventory_csv.php?center_code=$pc&search=Find&rep=x>Park</a> $export_all</th></tr>";
