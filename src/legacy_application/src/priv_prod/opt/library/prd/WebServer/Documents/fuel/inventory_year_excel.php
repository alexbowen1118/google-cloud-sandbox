...
// Get total mileage driven
 $sql= "SELECT t2.vehicle_id, (sum(t1.mileage) + t2.mileage) as tot_mileage
 FROM items as t1
...
//		echo "<tr><td>$sql</td></tr>";
		...
		foreach($ARRAY[0] as $k=>$v){
			...
			if($k=="mileage"){$k=$pass_year." mileage</th><th>total mileage";}
			echo "<th>$k</th>";
		}
		echo "</tr>";
		...
		foreach($ARRAY as $num=>$array){
			...
			foreach($array as $k=>$v)
				...
				if($k=="mileage")
					...
					$input.="</td><td>".@number_format($total_mileage_array[$array['vehicle_id']],0);
					}
				
				$td="left";
				if($k=="mileage"){$td="right";}
				echo "<td align='$td'>$input</td>";
				}
			echo "</tr>";
		}
