...
$menu['r_ytd']="r_ytd_day_multiyear_park_month.php";
$menuM=$month;
$menuY=$year;
if(!empty($year_1))
	{
...
$sql = "SELECT left(park,4) as park, left(year_month_day,4) as year,  substr(year_month_day,5,2) as month, format(sum(attend_tot),0) as visitation 

FROM `stats_day`

where left(year_month_day,4)>=$year_1 and left(year_month_day,4)<=$year_2 and park!='ARCH'

group by left(park,4), left(year_month_day,4), substr(year_month_day,5,2)

order by park, left(year_month_day,4)"; 
// echo "$sql";
...
if(@$xls=="excel")
	...
	header("Content-Disposition: attachment; filename=ncsp_year_park_month_visitation.csv");
	// Disable caching
