...
//	echo "<pre>"; print_r($_REQUEST); echo "</pre>";  //exit;
	if($period=="quarter1")
		...
		$ed = strtotime(date("Ymd", strtotime($start_date)) . " +3 month");
		$end_date=date("Ymd",$ed);
		$time_period="year_month_day>='$start_date' and year_month_day<'$end_date'";
		$start_month="January";
		$mid_month="February";
		$end_month="March";
...
if(mysqli_num_rows($result)<1)
	{echo "No attendance was recorded for $parkcode. Click your back button.";
	if($level>3){echo "$sql";}
 	exit;
	}
...
if($parkcode=="MOJE")
	{
	if($maxD>999 and $maxD<10000){$maxX=ceil($maxD+100);$bandIncrement=100;}
	}
else
	{
	if($maxD>999 and $maxD<10000){$maxX=ceil($maxD/1000)*1000;$bandIncrement=100;}
	}
	*/
if($maxD>999 and $maxD<10000){$maxX=ceil($maxD+100);$bandIncrement=100;}
...
@$Sname="North Carolina State Parks - Daily Attendance for $period $year";
...
for($i=1;$i<=count($ARRAY);$i++)
	...
	$month=substr($var_date,-4,2);
	$day=substr($var_date,-2,2);
	$year=substr($var_date,0,4);
...
pdf_setcolor ($pdf, 'both', 'rgb', 0, 0, 0, 0);
	...
	pdf_show_xy ($pdf, $start_month,63, 50);
	pdf_show_xy ($pdf, $mid_month,343, 50);
	pdf_show_xy ($pdf, $end_month,643, 50);


// Total for period

$pt=number_format($period_total,0);
@$text="Total for $period $year: $pt";
...
$xx=$xx+$barW+$gap;
    ...
    pdf_rotate($pdf, 90); /* rotate coordinates */
		pdf_show_xy ($pdf, number_format($numA,0),$a+85, -$xx-5);
    pdf_rotate($pdf, -90); /* rotate coordinates */;
		...
		//$link="http://www.dpr.ncparks.gov/attend/a/r_ytd.php?year=$year&month=$mon&submit=Enter";
			$starting_xpos = $xx;
			$starting_ypos = $a+83;
