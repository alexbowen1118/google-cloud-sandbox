...
$filename="NC_State_Park_Attendance_".$year.$month.".xls";
...
// echo "$sql"; exit;
$result = mysqli_query($connection,$sql) or die ("Couldn't execute query 1. $sql");
if(mysqli_num_rows($result)<1){echo "No monthly totals added yet.<br />"; exit;}
while ($row=mysqli_fetch_assoc($result))
	{
//	echo "<pre>"; print_r($row); echo "</pre>";  exit;
...
// echo "<pre>"; print_r($list_of_parks_array); echo "</pre>"; 
// echo "<pre>"; print_r($check_park_array_1); echo "</pre>";  exit;
...
//  	echo "<pre>"; print_r($incomplete_array); echo "</pre>";  
	...
	<input type='submit' name='submit_form' value=\"Ignore\">
	</td></tr>";
	echo "</table>";
	echo "</form>";
	exit;
...
if(@$submit=="Enter" or @$submit_form=="Ignore")
	...
	if(empty($xls))
		{echo "<td><a href='r_ytd_by_month_day_dncr_csv.php?submit_form=Ignore'>Excel export</td>";}
	echo "</tr>
